...
\item[Le microbiome\cite{Eisen}] est l'ensemble des génomes des micro-organismes provenant d'un environnement donné. Dans la plus part des cas, le microbiome fait référence aux bactéries seuls. on parle alors de virome et de mycobiome pour les virus et champignons.  
...
\item[Un OTU] (\textit{Operational Taxonomic Unit}) est un terme utilisé en phylogénie pour désigner un groupe d’individus semblable. Un OTU est très souvent considéré comme le nom de l'espèce dans la classification de Linné.
...
L’abondance relative est l'expression en pourcentage de l'abondance absolu, afin de rendre les echantillons comparable entre eux.

\item[La dominance] caractérise un OTU majoritaire dans un échantillon.


\item[La table des OTU] est la base de toutes analyses en écologie. Elle correspond à un tableau à double entrée contenant l’abondance par échantillon et par OTU. Dans le tableau \ref{OTUTABLE}, l'échantillon 1 contient 68\% de \textit{Staphylococcus Aureus}.
...
\item[La diversité alpha] est une mesure de biodiversité au sein d’un échantillon. Elle correspond à l’étude d’une colonne dans la table des OTUs. La richesse, l'indice de Chao1, de Shannon et de Simpson sont des indices de diversité alpha.
