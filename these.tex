...
\textsc{\normalsize Née le 18 Mai 1984} \\[0.1cm]
...
\end{center}
\caption{La mitochondrie: Symbiose ultime entre eucaryote et procaryote.}
\label{mitochondrie}
\end{figure}
...
\end{center}
\caption{Exemple de courbes de raréfactions.}
La courbe bleue témoigne d'un bon échantillonnage en s’aplatissant précocement. Plus de reads ne permettraient pas de découvrir de nouveaux OTUs contrairement à la courbe rouge.
...
\end{center}
\caption{Exemple d'une ordination]
...
\end{center}
\caption{Exemple d'un fichier FASTQ.}
...
\end{center}
\caption{Alignement du couple de reads sur l'ADNr 16S}le couple de séquence de 300 pb permet de recouvrir l'ensemble de la région V3-V5 de l'ARN 16S
\label{illumina}
\end{figure}
...
\end{center}
\caption{Qualité des des reads fusionnés pour l'échantillon 1003}
En abscisse la position du nucléotide dans le reads. En ordonnée la distribution des scores de qualité Phred.
\label{fastqt_after}
\end{figure}
...
\end{center}
\caption{Évolution des abondances par phylum. Les échantillon sont ordonnés par patient dans le temps}
...
\end{center}
\caption{Évolution des abondances par genre. Les échantillons sont ordonnés par patient dans le temps.}
